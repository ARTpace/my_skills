---
name: browser-use
description: "通过 browser-use CLI（`uvx browser-use`）实现的 AI 浏览器自动化。浏览网站、点击元素、填写表单、截图、提取数据和自动化 Web 任务。当用户需要与网页交互、抓取数据、填写表单、自动化工作流程或测试网站时使用。首次使用：运行`uvx browser-use install`来设置 Chromium。"
version: 1.0.0
