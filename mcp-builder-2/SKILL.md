---
name: mcp-builder
description: 创建高质量 MCP（模型上下文协议）服务器的指南，使 LLM 能够通过精心设计的工具与外部服务交互。适用于构建 MCP 服务器以集成外部 API 或服务，支持 Python（FastMCP）或 Node/TypeScript（MCP SDK）。
...
创建 MCP（模型上下文协议）服务器，使 LLM 能够通过精心设计的工具与外部服务交互。MCP 服务器的质量取决于它让 LLM 完成实际任务的能力。
...
**API 覆盖 vs. 工作流工具：**
在全面的 API 端点覆盖和专用的工作流工具之间取得平衡。工作流工具对特定任务更便捷，而全面覆盖让智能体能够灵活组合操作。不同客户端的性能表现各异——有些客户端受益于组合基础工具的代码执行，而另一些则更适合高层工作流。不确定时，优先选择全面的 API 覆盖。
...
**理解 API：**
查阅服务的 API 文档，识别关键端点、认证要求和数据模型。根据需要结合使用网络搜索和 WebFetch。
...
使用评估来测试 LLM 是否能有效利用你的 MCP 服务器回答真实、复杂的问题。
...
<evaluation>
  <qa_pair>
    <question>查找关于使用动物代号命名的 AI 模型发布的讨论。其中一个模型需要特定安全等级，格式为 ASL-X。以斑点野生猫科动物命名的模型正在确定哪个数字 X？</question>
    <answer>3</answer>
  </qa_pair>
<!-- 更多 qa_pairs... -->
</evaluation>
