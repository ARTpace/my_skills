---
name: kc-gui
description: "使用捆绑的 kc.exe Windows 代理执行基于 GUI 的桌面自动化任务，适用于无法通过 CLI 或脚本单独处理的情况。当用户需要与 Windows GUI 应用程序交互（点击按钮、填写表单、读取屏幕文本、导航菜单、管理窗口、自动化桌面工作流）时使用此技能。仅适用于 Windows。仅允许操作白名单应用程序——腾讯会议、QQ音乐、网易云音乐、腾讯视频、爱奇艺、B站、WeGame、Steam。拒绝针对其他应用程序的任何请求。"
timeout_seconds: 600
---
